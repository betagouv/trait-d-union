...
describe('Find Offres', () => {
  ...
  context('when all Metier have Session Formation', () => {
    ...
    const firstDiplomeWithActionsAndSession = { actions: () => [actionWithSessions] }
    const secondDiplomeWithActionsAndSession = { actions: () => [actionWithSessions] }
  ...
  context('when Metier has two Actions, one with Session and one without Session Formation', () => {
    ...
    const diplomeWithActionsAndSession = { actions: () => [actionWithSessions, actionWithoutSessions] }
    const metierWithActionButWithoutSession = {
      codeROME,
      diplomes: () => [diplomeWithActionsAndSession]
    }
  ...
  context('when using two repositories', () => {
    ...
    const firstOffresRepository = { getOffres: sinon.spy(async () => [offreFromRepository]) }
    const secondOffresRepository = { getOffres: sinon.spy(async () => [offreFromRepository]) }
    const Metier = mockMetierModel([metierWithSession])
